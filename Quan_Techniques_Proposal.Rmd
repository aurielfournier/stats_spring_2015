...
```{r, fig.width=4, fig.height=4, message=F, echo=F, warning=F, results='hide'}
...
ggplot()+
  ...
  xlim(-95.06,-95.04)+
  ylim(40.075, 40.11)+
  ggtitle("Three wetland units surveyed in 2014")
  
```

The data now is a marked point pattern across 13 different sites, 36 different wetland units (the unit of interest) and two years. Four observers surveyed across the two years, four in 2013, and two in 2014 (the two in 2014 were part of the 4 in 2013). Each point is marked with it's year, the date of the survey, the distance from the survey line and it's observer so that it can be subsetted to compare the point patterns from observer A and observer B on the same night in the same year. 
